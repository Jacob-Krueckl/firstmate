...
repair_line() {
  ...
  case "$HARNESS" in
    claude)
      ...
      ;;
    codex)
      printf '%s%s\n' "$prefix" 'restore the retained Codex owner with bin/fm-codex-watch.sh start THREAD PID, using this exact live TUI thread and home-lock owner; verify status before ending the turn.'
      ;;
    pi|pi-signed)
...
ordinary_wake_line() {
  case "$HARNESS" in
    claude)
      ...
      ;;
    codex)
      printf '%s\n' '- Ordinary wake: the retained Codex owner handles watcher continuity; drain and acknowledge the durable wake without starting another watcher.'
      ;;
    pi|pi-signed)
