...
Codex supervision follows the [retained TUI protocol](supervision-protocols/codex.md).
...
Claude depends on the Stop `asyncRewake` rewake, Cursor depends on its awaited stop-hook park, Grok retains native background-completion notifications; Codex limits are owned by its [supervision protocol](supervision-protocols/codex.md).
