...
test_selected_harness_block_only() {
  ...
  assert_contains "$out" "Mode: Codex retained TUI callback (Linux and tmux)." "codex snippet missing"
...
test_repair_lines() {
  ...
  assert_contains "$out" "bin/fm-codex-watch.sh start THREAD PID" "codex repair line did not use checkpoint helper and env override"
  ...
  assert_contains "$out" "bin/fm-codex-watch.sh start THREAD PID" "x-mode codex repair line lost the checkpoint helper"
...
test_cross_harness_ordinary_continuation_and_repair_matrix() {
  ...
  out=$("$RENDER" --harness codex)
  ordinary=$(printf '%s\n' "$out" | grep -F -- '- Ordinary wake:')
  assert_contains "$ordinary" "retained Codex owner" "codex ordinary-wake line lost its foreground checkpoint"
  assert_contains "$ordinary" "drain and acknowledge" "codex ordinary-wake line lost acknowledgement guidance"
  ...
  assert_contains "$out" "retained Codex owner" "codex recovery line lost retained-owner repair"
  assert_contains "$out" "bin/fm-codex-watch.sh" "codex recovery line lost owner command"
